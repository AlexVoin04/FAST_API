...
from fa_learn_app.utils.repository_utils import convert_products_storage_to_out, conver_storage_in_to_storage
...
class ProductTmpRepository(BaseProductRepository):
    ...
    def get_by_id(self, id :uuid.UUID) -> Optional[ProductOut]:
        ...
        if product is None:
            return None
        product_out :ProductOut = convert_products_storage_to_out(product)
        return product_out
    ...
    def get_all(self, limit: int, skip: int) -> List[ProductOut]:
        ...
        for _, product in self._dict_products.items():
            product_out_list.append(convert_products_storage_to_out(product))
        return product_out_list[skip:skip+limit]

    def create(self, product: ProductIn) -> ProductOut:
        """Создание продукта"""

        product_storage :ProductStorage = conver_storage_in_to_storage(product)
        self._dict_products.update({product_storage.id: product_storage})
        product_out :ProductOut = convert_products_storage_to_out(product_storage)
        return product_out